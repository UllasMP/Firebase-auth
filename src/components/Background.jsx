...
function Background({ bgOn, arcOn, ringsOn, hexOn, scanOn }) {
  ...
  const [particles] = useState(() => Array.from({ length: 22 }, (_, i) => ({
    ...
    px: ((Math.random() - 0.5) * 90).toFixed(1),
    sz: Math.random() > 0.72 ? 3 : 2,
    c: Math.random() > 0.8 ? "var(--red-b)" : Math.random() > 0.62 ? "var(--gold)" : "var(--cyan)",
  })));
      ...
      {particles.map((p) => (
        <div
          ...
          style={{
            ...
            left: `${p.left}%`,
            opacity: 0,
            animation: `bgPart ${p.dur}s linear ${p.dl}s infinite`,
            "--px": `${p.px}px`,
          }}
        />
      ))}
